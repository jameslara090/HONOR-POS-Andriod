...
const containerVariantClasses: Record<Variant, string> = {
  ...
  danger: 'bg-mod-accent-700 active:bg-mod-accent-800',
...
export function Button({ variant = 'primary', loading = false, disabled, trailing, children, ...props }: ButtonProps) {
  ...
  return (
    <Pressable
      disabled={isDisabled}
      className={`w-full min-h-[52px] flex-row items-center justify-between px-4 py-3 ${containerVariantClasses[variant]} ${isDisabled ? 'opacity-45' : ''}`}
      {...props}
    >
